---
permalink: /
title: "About"
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

I am **Kaibo Wang** (王凯渤), an MSc student in Artificial Intelligence at [King's College London](https://www.kcl.ac.uk/). I previously received a B.Eng. in Data Science and Big Data Technology from [Beijing Institute of Technology](https://english.bit.edu.cn/).

My work is on **information retrieval and recommender systems**, especially sequential / cross-domain ranking, LLM for RecSys, and spatiotemporal representation learning. I care about methods that still work under sparsity, distribution shift, and real-system constraints.

## Education

**King's College London**, MSc in Artificial Intelligence, Sep. 2025 -- Jan. 2027 (expected)  
Ranked top 10%. Master's project: SPR-Learning (abstract skill learning from long robot demonstrations).

**Beijing Institute of Technology**, B.Eng. in Data Science and Big Data Technology, Aug. 2021 -- Jun. 2025  
GPA 3.5/4.0. Outstanding Undergraduate Thesis.

## Selected publications

- **LeCDSR**: LLM-enhanced cross-domain sequential recommendation. *Information Fusion*, 2026 (SCI Q1). Only student author.
- **STG-Trip**: Spatial-temporal aware gating for out-of-town trip recommendation. To be submitted to WWW 2027. Only student author.

Full list on [Publications](/publications/). A one-page résumé is on [CV](/cv/).

## Research experience

**Research Assistant**, Beijing Institute of Technology, Dec. 2024 -- present  
Advisors: Prof. Shuliang Wang and Jiabao Zhu. Led method design, experiments, and writing for the two papers above as the only student author.

## Contact

Email: [wangkaibo72@gmail.com](mailto:wangkaibo72@gmail.com)
